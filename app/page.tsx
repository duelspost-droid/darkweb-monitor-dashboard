...
export default function SecurityPage() {
  ...
  return (
    <div className="space-y-7 pb-14">
      ...
      {/* 다크웹 인포스틸러 감염 (도메인 전수) */}
      <Panel
        title="다크웹 인포스틸러 감염 (도메인 전수)"
        subtitle="악성코드(인포스틸러)에 감염돼 자격증명이 탈취된 다크웹 스틸러 로그. 도메인 전체 집계."
        right={
          <span className="chip chip-neutral">
            <Bug size={13} className="mr-1 inline" aria-hidden /> 총 {infoTotal.toLocaleString()}건
          </span>
        }
      >
        {infostealer.length === 0 ? (
          <p className="py-6 text-center text-sm text-muted">인포스틸러 감염 데이터가 없습니다.</p>
        ) : (
                ...
                .map((i) => (
                  <div key={i.domain} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-muted">
                      <span>임직원 {i.employees}</span>
                      <span>· 사용자 {i.users}</span>
                      <span>· 서드파티 {i.thirdParties}</span>
                    </div>
                ...
                {sources.map((s) => (
                  <tr key={s.name} className="border-b border-slate-100 last:border-0">
                    <td className="px-3 py-2 font-semibold text-ink">
                      <Database size={13} className="mr-1 inline text-slate-400" aria-hidden />
                      {s.name}
                    </td>
                    <td className="px-3 py-2">
                      ...
                      >
                        {s.kind === "infostealer" ? "인포스틸러" : "데이터 유출"}
                      </span>
                    </td>
