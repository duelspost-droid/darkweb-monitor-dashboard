...
**최종 갱신: 2026-07-09** (직전: 07-06) — 최신 작업은 맨 아래 **19. 세션 로그**(노출 건별 조치 + 개인정보 탐지 확장) 참조. 07-04~05 세션은 **16b절**.

> 🚧 **2026-07-09 현재 이 서비스는 다운(Supabase 프로젝트 정지) 상태입니다.**
> 무료 org의 활성 프로젝트 2개 한도 때문에 이 프로젝트를 정지하고, 그 슬롯으로 5개 서비스가 얹힌
> 공유 프로젝트를 복구했습니다. darkweb은 **공유 프로젝트로 통합하는 중**(비용 $0, 슬롯 문제 영구 해소).
> **이어서 작업하려면 저장소 루트 [`HANDOFF-MIGRATION.md`](../HANDOFF-MIGRATION.md) 를 먼저 보세요** —
> 정지 전 전량 백업(444행·정책·함수·cron·시크릿) 위치와 재개 지점(스키마 적용부터)이 정리돼 있습니다.
